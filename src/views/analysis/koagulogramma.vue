<template>
  <div class="bg-white"  style="min-height: 100vh; border-radius:3px;">
    <div class="border-bottom d-flex justify-content-between">
      <router-link to="/koagulogramma">
         <h5 class="m-0 ml-4 d-flex" style="padding: 16px 0px">
           КОАГУЛОГРАММА</h5>
      </router-link>
      <div>
         <router-link to="/koagulogramma_add/0">
...
export default {
  data(){
    return {
      ...
      m_product :{
        rows: [],
        columns: ['patient_name', 'device_name','index_pt', 'vremya_pt','created_date_time', 'creator_doctor_name'],
        col : []
      },
    }
  },
  ...
  methods: {
    ...
    async checklist (){
      ...
      for_edit(edit_data)
      {
        this.$router.push("/koagulogramma_add/" + edit_data.id);
      },
      async for_delete(del_data)
      {
         const requestOptions = {
          ...
          const response = await fetch(this.$store.state.hostname + "/HospitalKoagulogramma/" + del_data.id, requestOptions);
      ...
      async refresh(){
        const res = await fetch(this.$store.state.hostname + '/HospitalKoagulogramma/getPagination?page=' + this.get_pagination.page + '&size=' + this.get_pagination.size);
