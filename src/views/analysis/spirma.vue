<template>
  <div class="bg-white"  style="min-height: 100vh; border-radius:3px;">
    <div class="border-bottom d-flex justify-content-between">
      <router-link to="/spirma">
         <h5 class="m-0 ml-4 d-flex" style="padding: 16px 0px">
           Спермаграмма</h5>
      </router-link>
      <div>
         <router-link to="/spirma_add/0">
...
export default {
  data(){
    return {
      ...
      m_product :{
        rows: [],
        columns: ['patient_name','name_1', 'name_2', 'name_3', 'created_date_time','creator_doctor_name'],
        col : []
      },
    }
  },
  ...
  methods: {
    ...
    async checklist (){
      ...
      for_edit(edit_data)
      {
        this.$router.push("/spirma_add/" + edit_data.id);
      },
      async for_delete(del_data)
      {
         const requestOptions = {
          ...
          const response = await fetch(this.$store.state.hostname + "/HospitalAnalizSperma/" + del_data.id, requestOptions);
      ...
      async refresh(){
        const res = await fetch(this.$store.state.hostname + '/HospitalAnalizSperma/getPagination?page=' + this.get_pagination.page + '&size=' + this.get_pagination.size);
