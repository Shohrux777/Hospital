...
export default {
  data(){
    return {
      ...
      m_product :{
        rows: [],
        columns: ['patient_name', 'device_name', 'ttg','estradiol','prolaktin', 'created_date_time','creator_doctor_name'],
        col : []
      },
    }
  },
  ...
  methods: {
    ...
    async checklist (){
      ...
      for_edit(edit_data)
      {
        this.$router.push("/garmonlarQon_add/" + edit_data.id);
      },
      async for_delete(del_data)
      {
         const requestOptions = {
          ...
          const response = await fetch(this.$store.state.hostname + "/HospitalQondagiGarmonlar/" + del_data.id, requestOptions);
      ...
      async refresh(){
        const res = await fetch(this.$store.state.hostname + '/HospitalQondagiGarmonlar/getPagination?page=' + this.get_pagination.page + '&size=' + this.get_pagination.size);
