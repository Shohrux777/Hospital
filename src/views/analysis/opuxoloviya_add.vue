<template>
  <div class="bakteriya bg-white" style="min-height: 100vh;">
    <div class="header text-center border-bottom py-2">
      <h4 class="text-primary">ОПУХОЛЕВЫЕ МАРКЕРЫ</h4>
    </div>
    <div class="row mt-2">
      <div class="col-4">
    ...
    <div v-else class="mx-1 row">
      <div class="col-6" >
        <div style="position:relative;">
          <mdb-input :label="('СА-125')"  v-model="name_1" outline/>
          <small v-if="$v.name_1.$dirty && !$v.name_1.required" class="invalid-text  pl-4" style="position:absolute; bottom:-18px; left:-15px;"> 
            {{$t('select_item')}}
          </small>
          <small class="pos_absolute">0-35 Ед/мл</small>
        </div>
  ...
  async created()
    {
      if(this.id > 0)
      {
        const res = await fetch(this.$store.state.hostname + '/HospitalAnaliz_1/' + this.id);
    ...
    async send(){
      try{
        ...
        if(data.id){
          ...
          this.$refs.message.success('Added_successfully')
          this.cls_wnd()
          this.$router.push('/opuxoloviya_print/' + data.id)
