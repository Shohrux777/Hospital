 ...
 <div>
   <!--Navbar-->
    <mdb-navbar class="flexible-navbar white border-bottom " style="box-shadow: 1px 0.5px 5px rgb(230,230,230)" light position="top" scrolling>
      <p class="navbar_brand ml-4">{{$t('menu_color')}}</p>
      <mdb-navbar-toggler>
        <mdb-navbar-nav right>
          <router-link to="/color_add/0">
...
export default {
  ...
  components: {
   ...
   data() {
    ...
    computed: mapGetters(['ErpcolorList']),
    methods:
    {
 ...mapActions(['fetchErpColor']),
 ...mapMutations(['color_delete_row']),
      ...
      async for_delete(del_data,index)
      {
         const requestOptions = {
          ...
          const response = await fetch(this.$store.state.hostname + "/ErpColors/" + del_data.id, requestOptions);
      ...
      for_edit(edit_data)
      {
        this.$router.push("/color_add/" + edit_data.id);
      },
      async update_column()
      {
        const res = await fetch(this.$store.state.hostname + '/color_column/' + this.user_id);
      ...
      async update_rows()
      {
           const res = await fetch(this.$store.state.hostname + '/ErpColors');
