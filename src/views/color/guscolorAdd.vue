   ...
   <!--Navbar-->
    <mdb-navbar class="flexible-navbar white border-bottom " style="box-shadow: 1px 0.5px 5px rgb(230,230,230)" light position="top" scrolling>
      ...
      <mdb-card-title class="font-weight-bold pt-2" :action="true"
        >{{$t('Add_color_density')}}</mdb-card-title>
    </mdb-navbar>
    <!--/.Navbar-->
    <main class="p-4 shadow" style="border-radius: 6px;">
      ...
      <form @submit.prevent="submit">
        <div class="container ">
          ...
          <hr class="mt-5"/>
          <mdb-btn  @click="submit_continue" color="info" m="r3"
...
import { required } from 'vuelidate/lib/validators'
  ...
  export default {
    ...
    async created()
    {
      if(this.id > 0)
      {
          const res = await fetch(this.$store.state.hostname + '/ErpColorGustotas/' + this.id);
    ...
    methods: {
      async submit_continue()
      {
        ...
        {
          if(this.id != 0){
            this.$router.push('/guscolor_add/0')
            this.cls_wnd();
          }
          else{
            this.cls_wnd();
      ...
      save_data :  async function()
      {
        ...
        const requestOptions = {
          ...
          const response = await fetch(this.$store.state.hostname + "/ErpColorGustotas", requestOptions);
