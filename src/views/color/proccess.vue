 ...
 <div>
   <!--Navbar-->
    <mdb-navbar class="flexible-navbar white border-bottom " style="box-shadow: 1px 0.5px 5px rgb(230,230,230)" light position="top" scrolling>
      <p class="navbar_brand ml-4">{{$t('menu_color_proccess')}}</p>
      <mdb-navbar-toggler>
        <mdb-navbar-nav right>
          <router-link to="/main_proccess_add/0">
...
export default {
  ...
  components: {
   ...
   data() {
    ...
    computed: mapGetters(['processGetList']),
    methods :
    {
      ...mapActions(['fetchprocessAll',]),
      ...mapMutations(['main_process_delete_row']),
       async for_delete(del_data,index)
      {
         const requestOptions = {
            method : "delete",
          };
          const response = await fetch(this.$store.state.hostname + "/ErpProccesses/" + del_data.id, requestOptions);
          ...
          if(data)
          {
            ...
            this.main_process_delete_row(index)
          }
          else{
      ...
      for_edit(edit_data)
      {
        this.$router.push("/main_proccess_add/" + edit_data.id);
      },
      async update_column()
      {
