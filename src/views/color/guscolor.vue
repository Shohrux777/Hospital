 ...
 <div>
   <!--Navbar-->
    <mdb-navbar class="flexible-navbar white border-bottom " style="box-shadow: 1px 0.5px 5px rgb(230,230,230)" light position="top" scrolling>
      <p class="navbar_brand ml-4" >{{$t('menu_color_density')}}</p>
      <mdb-navbar-toggler>
        <mdb-navbar-nav right>
          <router-link to="/guscolor_add/0">
...
export default {
  ...
  components: {
   ...
   data() {
    ...
    computed: mapGetters(['guscolor']),
     
    methods :
    {
      ...mapActions(['fetchColorGus',]),
      ...mapMutations(['gus_color_delete_row']),
       ...
       async for_delete(del_data,index)
      {
         const requestOptions = {
          ...
          const response = await fetch(this.$store.state.hostname + "/ErpColorGustotas/" + del_data.id, requestOptions);
          ...
          if(data)
          {
            ...
            this.gus_color_delete_row(index);
          }
          else{
      ...
      for_edit(edit_data)
      {
        this.$router.push("/guscolor_add/" + edit_data.id);
      },
      async update_column()
      {
      ...
      async update_rows()
      {
           const res = await fetch(this.$store.state.hostname + '/ErpColorGustotas');
