    ...
    <main style="border: 0.1px solid white; border-radius: 7px;" class="shadow">
      <form @submit.prevent="submit" >
        <div class="container-fluid ">
          ...
          <div class="ml-1">
            <mdb-btn slot="reference" class="mr-1 mt-4 px-4" outline="primary" @click="applied"
                      darkWaves tag="a" floating far icon="paper-plane"  size="sm" >{{$t('apply')}}</mdb-btn>
          </div>
          <!-- style="height : 400px"  -->
          <div class="d_table mt-3 p-1" v-if="sklad_submitted" style="position: relative;">
              ...
              <mdb-tbl-body>
                <tr  scope="row" v-for="(row,rowIndex) in datasource.rows" :key="rowIndex" >
                  ...
                  <th v-if="datasource.data[1].status">
                    <input v-model="row.qty"  @input="onChangeSumma(row,rowIndex)" type="number" class="form-control form-control-sm">
                    <small class="invalid-text" style="padding-top:26px; position:relative; margin:0"
                  ...
                  <th v-if="datasource.data[4].status"> <input v-model="row.price"  @input="onChangeSumma(row,rowIndex)" type="number" class="form-control form-control-sm"> </th>
                  <th v-if="datasource.data[5].status"> <input v-model="row.fein" type="number" class="form-control form-control-sm" disabled> </th>
                  <th v-if="datasource.data[6].status"> <input v-model="row.pus" type="number" class="form-control form-control-sm" disabled> </th>
                  <th v-if="datasource.data[7].status"> <input v-model="row.shirina" type="number" class="form-control form-control-sm" disabled> </th>
                  <th v-if="datasource.data[8].status"> <input v-model="row.gramm" type="number" class="form-control form-control-sm" disabled> </th>
                  <th v-if="datasource.data[9].status"> <input v-model="row.metraj" type="number" class="form-control form-control-sm" disabled> </th>
                  <th v-if="datasource.data[10].status"> <input v-model="row.qty2" type="number" class="form-control form-control-sm" disabled> </th>

                  <th v-if="datasource.data[11].status"> <input v-model="row.color_name" type="number" class="form-control form-control-sm" disabled> </th>
                  <th v-if="datasource.data[12].status"> <input v-model="row.color_name" type="number" class="form-control form-control-sm" disabled> </th>
                  <th v-if="datasource.data[13].status"> <input v-model="row.gus_color_name" type="number" class="form-control form-control-sm" disabled> </th>
                  <th v-if="datasource.data[14].status"> <input v-model="row.suroviy_vid_name" type="number" class="form-control form-control-sm" disabled> </th>
                  <th v-if="datasource.data[15].status"> <input v-model="row.note" type="text" class="form-control form-control-sm"> </th>

                  <th v-if="datasource.data[16].status"> <input v-model="row.main_proccess_name" type="number" class="form-control form-control-sm" disabled> </th>
                  <th v-if="datasource.data[17].status"> <input v-model="row.dop_work_name" type="number" class="form-control form-control-sm" disabled> </th>
                  ...
                  <th v-if="datasource.data[18].status">
                   ...
                   <th v-if="datasource.data[19].status"> <input v-model="row.sort_name" type="number" class="form-control form-control-sm" disabled> </th>
                  ...
                  <th v-if="datasource.data[21].status"> <input v-model="row.summ" type="number" class="form-control form-control-sm" disabled> </th>
                  <th v-if="datasource.data[22].status"> <input v-model="row.brutto" type="number" class="form-control form-control-sm" disabled> </th>
                  <th v-if="datasource.data[23].status"> <input v-model="row.netto" type="number" class="form-control form-control-sm" disabled> </th>
                  <th v-if="datasource.data[24].status"> <input v-model="row.upakovka_name" type="number" class="form-control form-control-sm" disabled> </th>
          ...
          <hr class=" mt-5 "/>
          <div v-if="sklad_submitted" class="d-flex justify-content-end">
  ...
  export default {
    ...
    data(){
      return {
        ...
        calc_type :{ selected: 'calc_type_qty1',selectedId:1,},
        ...
        datasource :
        {
          rows :[],
          rowsName : [],
          columns: ['Продукт', 'Кол-во', 'Остаток', 'Ед. изм.','Цена', 'Фейн','Пус','Ширина','Граммаж','Метрадж','Кол-во2','Цвет','Вариант цвета','Густота цвета', 'Суровый вид','Примечание','Основной процесс','Доп. работа','Дефект','Сорт','Угар','Сумма','Брутто','Нетто','Упаковка'],
          col : [],
          data : [
          ],
          filtered_data:[
    ...
    methods: {
      ...mapActions(['fetch_companylist','fetchKontragets','fetchDepartments','fetch_sklad_list_id',
      ...
      // ---> Summani hisoblash <---
      onChangeSumma(row,rowIndex)
      {
         this.datasource.rows[rowIndex].summa = row.price * row.soni;
      },
      // ---> Summani hisoblash <---
      ...
      selectproduct(option)
      {
        ...
        this.datasource.rows[option.row].erpInvoiceTypeId = this.invType.id;
     ...
     async submit()
      {
        ...
        for(let i=0;i<this.datasource.rows.length; i++){
          ...
          this.for_base.erpProductId = this.datasource.rows[i].erpProductId;
          this.for_base.qty = this.datasource.rows[i].qty;
          this.for_base.erpInvoiceTypeId = 4;
          this.for_base.realCount = this.datasource.rows[i].qty;
          this.for_base.price = this.datasource.rows[i].price;
          this.for_base.fein = this.datasource.rows[i].fein;
          this.for_base.pus = this.datasource.rows[i].pus;
          this.for_base.shirina = this.datasource.rows[i].shirina;
          this.for_base.gramm = this.datasource.rows[i].gramm;
          this.for_base.metraj = this.datasource.rows[i].metraj;
          this.for_base.qty2 = this.datasource.rows[i].qty2;
          this.for_base.brack = this.datasource.rows[i].brack;
          this.for_base.erpColorId = this.datasource.rows[i].erpColorId;
          this.for_base.erpColorVariantId = this.datasource.rows[i].erpColorVariantId;
          this.for_base.erpColorGustotaId = this.datasource.rows[i].erpColorGustotaId;
          this.for_base.erpSuroviyVidId = this.datasource.rows[i].erpSuroviyVidId;
          this.for_base.note = this.datasource.rows[i].note;
          this.for_base.erpProccessId = this.datasource.rows[i].erpProccessId;
          this.for_base.erpExtraWorkId = this.datasource.rows[i].erpExtraWorkId;
          this.for_base.erpSortId = this.datasource.rows[i].erpSortId;
          this.for_base.ugar = this.datasource.rows[i].ugar;
          this.for_base.brutto = this.datasource.rows[i].brutto;
          this.for_base.netto = this.datasource.rows[i].netto;
          this.for_base.erpUpakopkaId = this.datasource.rows[i].erpUpakopkaId;
          this.dataItems.push(this.for_base);
        ...
        const requestOptions = {
            ...
            body: JSON.stringify({
            "dateTime" : this.time1.toLocaleString("es-CL"),
            "activeStatus" : true,
            "createdUserId" : 1,
            "updatedUserId" : 1,
            "acceptedUser": false,
            "invoiceTypeStr": this.invType.name,
            "createdDateTime" : this.time1.toLocaleString("es-CL"),
            "count" : this.datasource.rows.reduce((a, b) => +a + +b.qty, 0),
            "summ": this.datasource.rows.reduce((a, b) => +a + +b.summ, 0),
            // "realCount": this.datasource.rows.reduce((a, b) => +a + +b.summ, 0),
            ...
            "filialCompanyId" : this.company_id,
            "departmentId" : this.dept_id,
            "erpWarehouseId" : this.sklad_id,
            "erpInvoiceTypeId": this.invType.id,
   ...
   async mounted (){
        const res = await fetch(this.$store.state.hostname + '/ErpInvoiceTypes/getSaleInvoiceType');
        this.invType = await res.json();
    ...
    computed: mapGetters(['get_filial_list','get_postavchik_list','getDepartId','get_sklad',
      ...
      'upakovka_list','sort_list','CurrencyList']),
