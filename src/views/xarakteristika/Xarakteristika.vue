 ...
 <div>
   <!--Navbar-->
    <mdb-navbar class="flexible-navbar white border-bottom " style="box-shadow: 1px 0.5px 5px rgb(230,230,230)" light position="top" scrolling>
      <p class="navbar_brand ml-4">{{$t('menu_xr')}}</p>
      <mdb-navbar-toggler>

        <mdb-navbar-nav right>
          <router-link to="/xarakteristika_add/0">
...
export default {
  ...
  components: {
   ...
   data() {
    ...
    computed: mapGetters(['CharacterList']),
    methods:{
      ...mapActions(['fetchCharacter','fetchCol']),
      ...mapMutations(['xr_delete_row']),

      for_edit(edit_data)
      {
        this.$router.push("/xarakteristika_add/" + edit_data.id);
      },

      // ----for Настройки вида-----//
     async checklist (){
        this.loading = true;
          await this.fetchCol(this.user_id);
        this.loading = false;
      },
      async for_delete(del_data,index)
      {
         const requestOptions = {
          ...
          const response = await fetch(this.$store.state.hostname + "/ErpCharacteristics/" + del_data.id, requestOptions);
