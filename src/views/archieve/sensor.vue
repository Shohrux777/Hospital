  ...
  <div>
      ...
      <div class="bg-white p-4 mb-5 pt-4 shadow" style="border-radius:5px; position:relative;">
        <div style="height: 60px;" class="d-flex justify-content-between border-bottom align-items-center  ">
          <div class="title d-flex align-items-center">
            <h5 class="m-0 ">Датчик таблица</h5>
          </div>
          <div class="plus">
    ...
    <!-- <div :class="{'showing':show}">
      ...
      <mdb-modal  :show="show"  @close="show = false"  light>
        <mdb-modal-header>
          <mdb-modal-title style="font-weight:  500;">{{$t('Добавить датчик')}}</mdb-modal-title>
        </mdb-modal-header>
        <mdb-modal-body>
          <sensorAdd :options="editData"/>
        </mdb-modal-body>
      </mdb-modal>
...
<script>
  import sensorAdd from "../../components/archieve_add/sensor_add"
  ...
  export default {
    ...
    computed: mapGetters(['get_sensor_list']),
    methods: {
      ...mapActions(['fetch_sensor']),
      ...mapMutations(['sensor_row_delete']),
      ...
      async for_delete(del_data,index){
          ...
          const response = await fetch(this.$store.state.hostname + "/Datchiks/" + del_data.id, requestOptions);
