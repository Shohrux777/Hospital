    ...
    <div class="rv_lab allWidthLab" id="table" style="position:relative;">
      ...
      <div class="lab_answer upHeight">
        <div class=" " style="display:flex; justify-content:center; flex-direction:column;">
          <h3 style="width:100%; text-align:center; font-weight:bold; font-size:55px;" class="mb-5">ГЕПАТИТ В,С (ИФА)</h3>
          ...
          <table border class="allWidthLabTable" style="margin-top:35px;">
            <thead>
              <tr>
                <th class="fs font-weight-bold py-2">Наименование</th>
            ...
            <tbody class="fs">
              <tr>
                <td class="fs">Гепатит A HAV</td>
                <td class="fs">{{option.a}}</td>
                <td class="fs"></td>
              </tr>
              <tr>
                <td class="fs">Гепатит В НBsAg</td>
                <td class="fs">{{option.hbsag}}</td>
                <td class="fs">0.198 </td>
              </tr>
              <tr>
                <td class="fs">Гепатит C Anti-HGV</td>
                <td class="fs">{{option.anti_hgv}}</td>
                <td class="fs">0.194</td>
              </tr>
              <tr>
                <td class="fs">Гепатит D HDV</td>
                <td class="fs">{{option.d}}</td>
                <td class="fs"></td>
              </tr>
            </tbody>
          </table>
...
export default {
 async mounted() {
  ...
  async created(){
    if(this.id>0){
      this.loading =true;
      const response = await fetch(this.$store.state.hostname + '/HospitalGepatitBC/' + this.id)
