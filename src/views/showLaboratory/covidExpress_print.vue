<template>
  <div>
    <div class="rv_lab allWidthLab" id="table" style="position:relative;">
      <input type="text" style="position:absolute; top: -500px; outline:none;border:none;" ref="printer" @keyup.enter="send">
      ...
      <div class="lab_answer upHeight"  >
        <div class=" " style="display:flex; justify-content:center; flex-direction:column;">
          <div style="width:100%;">
            <h3 class="headerTitle" style=" text-align:center; font-weight:bold;">Express COVID-19 Antigen </h3>
          ...
          <table border  class="allWidthLabTable"  >
            <thead >
              ...
              <tr>
                ...
                <th class="fs">COVID-19 and Influenza A+B Antigen Combo Rapid Test (Nasopharyngeal Swab)</th>
              </tr>
              <tr>
                <th class="fs">5</th>
                <th class="fs">Лаборатор таҳлил натижаси / The result of laboratory analysis</th>
                <th class="fs">{{option.result}}</th>
              </tr>
              <tr>
                <th class="fs">6</th>
...
export default {
 async mounted() {
  ...
  async created(){
    if(this.id>0){
      this.loading =true;
      const response = await fetch(this.$store.state.hostname + '/HospitalCovidExpress/' + this.id)
  ...
  methods: {
    ...
    async send(){
      ...
      const requestOptions = {
        ...
        "sended_status":  true,
        "doctor_name": 'covidExpress_print',
