  ...
  <div class="MainOrder ">
    ...
    <div v-if="!loading">
        <div style="height:100vh; margin-top: -70px;" class="d-flex align-items-center justify-content-center" 
         v-if="sub_product_main_id==0 ">
          <h5>Hech nima yuq</h5>
        </div>
      </div>
    <mdb-container fluid v-if="!loading">
      <mdb-row class="px-1 pb-4">
        <mdb-col sm="4" md="3" lg="2" class="itemPro "  v-for="(item,i) in sub_product_main_id" :key="i">
          <div @click="selectCategory(item)">
            <div  class="card rounded mt-4 shopCard shadow" >
              <div class="picture" style="position: relative;">
                <mdb-view class="zoom overlay z-depth-1" :src="item.pictureStr" alt="zoom">
                  <mdb-mask flex-center :text="item.name" />
                </mdb-view>
              </div>
              <div class="title  border-top" >
                <p  class="mt-2 mb-1 ml-2 text-dark">{{item.name}}</p>
                <div class="row">
                   <div class="col-12 ">
                    <p class=" text-right m-0 mr-2 text-info pb-1" style="font-size: 13px;">{{item.price}} 6 ведов</p>
                  </div>
                </div>
...
export default {
  ...
  async mounted(){
    console.log(this.id)
    await this.fetch_main_group_id(this.id);
    this.loading = false;
   
    // console.log(this.get_m_product_list)
  },
  methods: {
      ...mapActions(['fetch_main_group_id']),
      ...mapMutations(['add_product_order']),
      selectCategory(data){
        console.log(data)
        this.$router.push({name: 'main_order', params: {id: data.id}, query: {product:data.name}})
        // , query: {product:data.name}
      }
  },
  computed: mapGetters(['sub_product_main_id'])
