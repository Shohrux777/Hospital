...
<script>
  ...
  export default {
    ...
    computed: mapGetters(['get_agent_list']),
    methods: {
      ...mapActions(['fetch_m_agent']),
      ...mapMutations(['m_agent_row_delete']),

      for_edit(data){
        this.$router.push('/agent_add/'+ data.id)
      },
      async for_delete(del_data,index){
          const requestOptions = {
            method : "delete",
          };
          const response = await fetch(this.$store.state.hostname + "/MarketAgents/" + del_data.id, requestOptions);
