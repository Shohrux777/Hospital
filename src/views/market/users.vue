  ...
  <div>
    ...
    <!-- <div :class="{'showing':show}">
      ...
      <mdb-modal  :show="show"  @close="show = false" size="lg" light>
        ...
        <mdb-modal-body>
          <usersAdd :options="editData" @close="show = false"/>
        </mdb-modal-body>
      </mdb-modal>
...
<script>
  ...
  // import ModalKorzinka from '../../components/modal'
  import anyTable from "../../components/userTable"
  ...
  export default {
    components: {
      mdbBtn,
      mdbIcon,
      anyTable, mdbModal, mdbModalHeader, mdbModalTitle, mdbModalBody,
      usersAdd
    ...
    computed: mapGetters(['get_users_list', 'get_pagination']),
    methods: {
      ...mapActions(['fetch_users']),
      ...mapMutations(['users_row_delete', 'update_pagination_first']),
      ...
      async refresh(){
        const res = await fetch(this.$store.state.hostname + '/Users/getPagination?page=' + this.get_pagination.page + '&size=' + this.get_pagination.size);
        ...
        this.m_users.rows = res_data.items_list;
        this.loading = false;
      }
