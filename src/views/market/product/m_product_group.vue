  ...
  <div>
      ...
      <div class="bg-white p-4 mb-5 pt-4 shadow" style="border-radius:5px; position:relative;">
        <div style="height: 60px;" class="d-flex justify-content-between border-bottom align-items-center  ">
          <div class="title d-flex align-items-center">
            <h5 class="m-0 ">{{$t('category table')}}</h5>
          </div>
          <div class="plus">
    ...
    <!-- <div :class="{'showing':show}">
      ...
      <mdb-modal  :show="show"  @close="show = false"  light>
        <mdb-modal-header>
          <mdb-modal-title style="font-weight:  500;">{{$t('Add category')}}</mdb-modal-title>
        </mdb-modal-header>
        <mdb-modal-body>
...
<script>
  ...
  export default {
    ...
    computed: mapGetters(['m_group_list_m']),
    methods: {
      ...mapActions(['fetch_m_group']),
      ...mapMutations(['m_product_group_row_delete']),

      for_edit(data){
        this.$router.push('/m_product_group_add/'+ data.id)
      },
      async for_delete(del_data,index){
          const requestOptions = {
            method : "delete",
          };
          const response = await fetch(this.$store.state.hostname + "/MarketProductGroups/" + del_data.id, requestOptions);
          ...
          if(data)
          {
            ...
            this.m_product_group_row_delete(index);
          }
          else{
      ...
      add(){
        // this.show =! this.show
        // this.editData = {};
        this.$router.push('/m_product_group_add/0')
      }
