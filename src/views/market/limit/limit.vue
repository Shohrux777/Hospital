  ...
  <div>
    ...
    <!-- <div :class="{'showing':show}">
      ...
      <mdb-modal  :show="show"  @close="show = false"  light>
        <mdb-modal-header>
          <mdb-modal-title style="font-weight:  500;">{{$t('Add Agent')}}</mdb-modal-title>
        </mdb-modal-header>
        <mdb-modal-body>
...
<script>
  ...
  export default {
    ...
    async mounted(){
      await this.fetch_m_limit();
    },
    computed: mapGetters(['get_limit_list']),
    methods: {
      ...mapActions(['fetch_m_limit']),
      ...mapMutations(['m_limit_row_delete']),

      for_edit(data){
        this.$router.push('/limit_add/'+ data.id)
      },
      async for_delete(del_data,index){
          const requestOptions = {
            method : "delete",
          };
          const response = await fetch(this.$store.state.hostname + "/MarketAuthLimitByMoneys/" + del_data.id, requestOptions);
