   ...
   <loader v-if="loading"/>
    <form v-on:submit.prevent="submit">
      <div class="addxizmat px-2 ">
        <div class="row">
          <div class="col-12">
            <mdb-input label="Число порядка услуги" v-model="name" outline/>
  ...
  async mounted(){
    ...
    methods: {
      ...
      async submit(){
          ...
          this.loading = true;
           const response = await fetch(this.$store.state.hostname + '/PatientServiceTypes/addPatientServiceType?Id=' + this.id + '&Name=' + this.name)
