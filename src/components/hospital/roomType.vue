        ...
        <div class="container ">
          <mdb-row >
            <mdb-col class="col-12 col-sm-12 col-md-12 col-lg-12">
              <mdb-input v-model="rt_name"  size="sm" :label="$t('InputAddIzm')" icon="file-signature" group type="text" validate error="wrong" success="right"/>
              <small class="invalid-text pt-2" style="margin-left: 30px;"  v-if="$v.rt_name.$dirty && !$v.rt_name.required" >
                {{$t('name_invalid_text')}}
              </small>
            </mdb-col>
    ...
    methods : {
       ...
       save_data :  async function()
      {
        ...
        const requestOptions = {
          ...
          const response = await fetch(this.$store.state.hostname + "/MarketUnitMeasurments", requestOptions);
          const data = await response.json();
          if(data.name)
          {
            this.$refs.message.success('Added_successfully')
            this.fetch_m_Measure();
            return true;
          }
          else{
