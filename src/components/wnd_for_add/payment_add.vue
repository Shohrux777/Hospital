        ...
        <div class="container ">
          <mdb-row >
            <mdb-col class="col-12 col-sm-12" v-bind:class="{'col-md-8 col-lg-6':notFull}">
              <mdb-input v-model="x_name"  size="sm" :label="$t('InputAddPayment')" icon="file-signature" group type="text" validate error="wrong" success="right"/>
              <small class="invalid-text pt-2" style="margin-left: 30px;"  v-if="$v.x_name.$dirty && !$v.x_name.required">
                {{$t('name_invalid_text')}}
              </small>
            </mdb-col>
            <mdb-col class="col-12 col-sm-12 mt-2" v-bind:class="{'col-md-8 col-lg-6':notFull}" >
...
import { required } from 'vuelidate/lib/validators'
  ...
  export default {
    ...
    methods : {
      ...mapActions(['fetchpaymentTypeAll',]),
      ...mapMutations(['onSelect_xrList']),
      cls_wnd()
      {
        this.x_name = ''
        this.fp_name = ''
       ...
       save_data :  async function()
      {
        ...
        const requestOptions = {
          ...
          const response = await fetch(this.$store.state.hostname + "/ErpPaymentTypes", requestOptions);
          const data = await response.json();
          if(data.name)
          {
            this.$refs.message.success('Added_successfully')
            this.fetchpaymentTypeAll();
            return true;
          }
          else{
