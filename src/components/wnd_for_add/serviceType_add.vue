        ...
        <div class="container ">
          <mdb-row >
            <mdb-col class="col-12 col-sm-12" v-bind:class="{'col-md-8 col-lg-6':notFull}">
              <mdb-input v-model="x_name"  size="sm" :label="$t('InputAddServicceType')" icon="file-signature" group type="text" validate error="wrong" success="right"/>
...
import { required } from 'vuelidate/lib/validators'
  ...
  export default {
    ...
    methods : {
      ...mapActions(['fetch_service_type_list',]),
       ...
       save_data :  async function()
      {
        ...
        const requestOptions = {
          ...
          const response = await fetch(this.$store.state.hostname + "/ErpServiceTypes", requestOptions);
