        ...
        <div class="container ">
          <mdb-row >
            <mdb-col class="col-12 col-sm-12 col-md-6 col-lg-6">
              <mdb-input v-model="x_name"  size="sm" :label="$t('InputAddIzm')" icon="file-signature" group type="text" validate error="wrong" success="right"/>
...
import { required } from 'vuelidate/lib/validators'
  ...
  export default {
    ...
    methods : {
      ...mapActions(['fetch_poduct_type','fetchMeasure']),
      ...mapMutations(['onSelect_xrList','onSelect_selected_xr_list']),
      cls_wnd()
      {
        this.x_name = ''
        this.fp_name = ''
       ...
       save_data :  async function()
      {
        ...
        const requestOptions = {
          ...
          const response = await fetch(this.$store.state.hostname + "/ErpUnitmeasurments", requestOptions);
