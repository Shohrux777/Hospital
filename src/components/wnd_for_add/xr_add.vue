...
// import localizeFilter from '../../filters/localize.filter';
  ...
  export default {
    ...
    async created()
    {
      if(this.id > 0)
      {
          const res = await fetch(this.$store.state.hostname + '/ErpCharacteristics/' + this.id);
    ...
    methods : {
       ...
       save_data :  async function()
      {
        ...
        const requestOptions = {
          ...
          const response = await fetch(this.$store.state.hostname + "/ErpCharacteristics", requestOptions);
      ...
      async submit()
      {
          if(await this.save_data())
          {
              this.$router.push("/xarakteristika");
          }
      }
    },
