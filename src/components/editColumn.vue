      ...
      <div class="card_check">
        ...
        <div class="form_card border mt-4">
          ...
          <div class="form_right" v-show="listCheckshow">
            ...
            <div class="check_hover custom-control custom-checkbox" v-for="(see,i) in option.col.columnListJson" :key="i">
              <div class="check_dev">
          ...
          <div class="form_right" v-show="listEditshow">
            ...
            <div class="check_hover" v-for="(see,i) in option.col.columnListJson" :key="i">
              <div class="check_dev">
...
export default {
  ...
  mounted (){
    console.log( this.option.col)
    for(let i=0; i<this.option.length; i++){
      this.list_data.push(this.option[i].status);
    }
      this.op_len = this.option.col.columnListJson.length
    ...
    all_func(x){
        ...
        if(x === false){
          for(let i=0; i<this.op_len; i++){
            this.option.col.columnListJson[i].status = false
          }
        }else{
          for(let i=0; i<this.op_len; i++){
            this.option.col.columnListJson[i].status = true
          }
        }
      },
      cancel (){
        for(let i=0; i<this.op_len; i++){
          this.option.col.columnListJson[i].status = this.list_data[i]
        }
       ...
       // console.log(this.option);
        ...
        const requestOptions = {
            ...
            body: JSON.stringify({
              ...
              "columns" : JSON.stringify(this.option.col.columnListJson)
            })
          };

         const response = await fetch(this.$store.state.hostname + "/update_col", requestOptions);
